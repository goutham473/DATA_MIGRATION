package datapipeline.model.process
...
object Process {
  ...
  def invokeDataLoading(driver: Driver)(sparkSession: SparkSession) {
    ...
    driver.dataPipeLineProps.tables.foreach{ tables =>
      ...
      val prepareStas = prepareStatsInfo(sparkSession, baseTableDf, backUpTableDf, totalUpdates, totalDeletes, totalInserts, tables.tableInfo._1.baseTable)(brodCastMaxRowNumberStats.value)

      Write.loadToHive(tables.tableInfo._1.historyTable, seqIdColumn)(prepareHistoryData)(sparkSession)
      Write.loadToHive(driver.dataPipeLineProps.statusTable, "seq_Id")(prepareStas)(sparkSession)
    }
  ...
  def findInserts(baseTableDf: DataFrame, backupTableDf: DataFrame, keyColumns: List[String]): DataFrame = {
  ...
  def findDeletes(baseTableDf: DataFrame, backupTableDf: DataFrame, keyColumns: List[String]):DataFrame = {
  ...
  def findUpdates(baseTableDf: DataFrame, backupTableDf: DataFrame, keyColumns:List[String]): DataFrame = {
    val joinExprForMatch = keyColumns.map{col => baseTableDf(col) === backupTableDf(col)}.reduce(_&&_)
  ...
  def prepareStatsInfo(spark: SparkSession, baseTableDf: DataFrame, backUpTableDf: DataFrame, totalUpdates: DataFrame, totalDeletes: DataFrame, totalInserts: DataFrame, tableName: String)(maxRowNumber: Long): DataFrame = {
    ...
    val data_DF = data.withColumn("start_date",current_timestamp).withColumn("end_date",current_timestamp).withColumn("last_modified_dt",current_timestamp).withColumn("Job_Name",lit(s"job_${tableName}_name"))
    val addSeq = spark.
